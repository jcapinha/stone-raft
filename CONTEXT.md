...
Hello then sleep before the codec. No live scope and no patch card on the Seed. Blocking I2C (SPI4 masked) is only used before audio. Audio DMA uses the daisy-embassy default stash. A later patch card or menus must not pause the audio interrupt. Hardware is deferred (another microcontroller, or Daisy with a non-blocking display path).
